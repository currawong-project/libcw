...
protected:
    ...
    void TearDown() override {
        ...
        // Remove backup files generated by backup function in current dir and backup dir
        for (unsigned i = 0; i < 10; ++i) {
            char buf[128];
            snprintf(buf, sizeof(buf), "test_backup_%u.txt", i);
            remove(buf);
            snprintf(buf, sizeof(buf), "%s/test_backup_%u.txt", test_backup_dir, i);
            remove(buf);
        }
        filesys::rmDir(test_backup_dir);
    }
...
TEST_F(FileTest, Backup) {
    ...
    ASSERT_EQ(rc, kOkRC);
    EXPECT_TRUE(filesys::isFile("test_backup_dir/test_backup_0.txt")); 
    EXPECT_STREQ(readFileToString("test_backup_dir/test_backup_0.txt").c_str(), content);
