...
int main( int argc, char** argv )
{
  ...
  if((rc = obj->getv("io_test",obj)) != kOkRC )
  {
    printf("The 'io_test' object was not found in the cfg. file '%s'.",cwStringNullGuard(argv[1]));
    goto errLabel;
  }
  
  if((rc = io::test(obj)) != kOkRC )
  {
    printf("The 'io_test' application reported an error '%i' on exit.",rc);
    goto errLabel;
  }
