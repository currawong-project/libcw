...
int main( int argc, char** argv )
{
  ...
  if( argc < 2 )
  {
    printf("Usage: audio_midi <cfg_file>\n");
    rc = kSyntaxErrorRC;
    goto errLabel;
  }
  ...
  if((rc = obj->getv("audio_midi",obj)) != kOkRC )
  {
    printf("The 'audio_midi' object was not found in the cfg. file '%s'.",cwStringNullGuard(argv[1]));
    goto errLabel;
  }
  
  if((rc = audio_midi_app::main(obj)) != kOkRC )
  {
    printf("The 'audio_midi' application reported an error '%i' on exit.",rc);
    goto errLabel;
  }
