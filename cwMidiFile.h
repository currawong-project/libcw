...
namespace cw
{
  namespace midi
  {
    ...
    namespace file
    {
      ...
      // Columns: "UID","trk","dtick","atick","amicro","type","ch","D0","D1"
      rc_t open_csv_2( handle_t& hRef, const char* csv_fname );

      // Create a single track MIDI file with a fixed tempo.
      // Columns: sec,ch,status,d0,d1
      rc_t open_csv_3( handle_t& hRef, const char* csv_fname, unsigned bpm=120, unsigned ticksPerQN=1260 );
